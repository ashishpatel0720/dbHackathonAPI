...
public interface TravelActivityRepository extends CrudRepository<TravelActivity, Integer> {
    ...
    @Transactional
    @Modifying
    @Query(value="update travel_activity t set t.medium=:medium, t.fuel_type =:fuelType ,t.distance =:distance, t.contributors =:contributors , t.ghg_footprint=:ghgFootprint where t.id=:id",nativeQuery = true)
    void modifyUserInfoById(@Param("medium") String medium, @Param("fuelType") String fuelType, @Param("contributors") int contributors,@Param("distance") String distance, @Param("ghgFootprint") String ghg_footprint, @Param("id") int id);
